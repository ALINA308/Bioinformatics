"""
INFLUENZA GENOME MOTIF SCANNER
Scans 10 influenza genomes for exon-intron boundary motifs
Creates visualization charts showing most likely functional motif locations
"""
...
def scan_sequence(sequence, log_matrix, motif_length=9):
    """Scan a sequence for motifs using sliding window"""
    scores = []
    ...
    for i in range(len(sequence) - motif_length + 1):
        ...
        for pos, nuc in enumerate(window):
            if nuc in log_matrix:
                ...
                # Handle ambiguous nucleotides (N, etc.)
                valid = False
                break
        ...
        if valid:
            ...
            scores.append({
                'position': i,
                'window': window,
                'score': -10  # Very low score for invalid windows
            })

    return scores

def identify_significant_motifs(scores, threshold=0):
    """Identify significant motif locations above threshold"""
    significant = []
...
def create_genome_chart(genome_data, scores, output_filename):
    """Create visualization chart for genome showing motif locations"""

    sequence = genome_data['sequence']
    genome_id = genome_data['id']
    description = genome_data['description']

    # Extract score values for plotting
    ...
    # Subplot 1: Score plot along genome
    ...
    # Subplot 2: Genome map with motif locations
    ax2.set_xlim(0, len(sequence))
    ax2.set_ylim(0, 1)

    # Draw genome as a line
    ax2.plot([0, len(sequence)], [0.5, 0.5], 'k-', linewidth=3, label='Genome')

    # Mark significant motif locations
    if significant:
...
def generate_summary_report(all_results):
    """Generate summary report for all genomes"""
...
def main():
    ...
    print("="*80)
    print(f"\n[OK] Analyzed {len(genomes)} influenza genomes")
    print(f"[OK] Generated {len(genomes)} visualization charts")
    print(f"\nCharts saved as: genome_*_motif_chart.png")
    print("\n" + "="*80 + "\n")
